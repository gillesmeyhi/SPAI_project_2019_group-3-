...
//AudioBufferFIFO<float> abuffer2(1, 1920);
...
void MainComponent::run()
{
	...
	AudioBuffer <float> udpBuffer2;
	udpBuffer2 = juce::AudioBuffer<float>(1, 9600);
	udpBuffer.clear();
	udpBuffer2.clear();
	while (!threadShouldExit())
	{
		...
		//FOR CHANNEL1
		if (toggle.getToggleState() == true) {
			sock1.read((void*)udpBuffer.getWritePointer(0, 0), 7680, true);
			//sock2.read((void*)udpBuffer2.getWritePointer(0, 0), 7680, true);
			
			
		}
		else { 
			sock2.read((void*)udpBuffer.getWritePointer(0, 0), 7680, true); 
			//sock1.read((void*)udpBuffer2.getWritePointer(0, 0), 7680, true);
...
void MainComponent::resized()
{
	...
	toggle.setBounds(100, 300, getWidth() - 110, 20);
